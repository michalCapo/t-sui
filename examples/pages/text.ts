...
export function TextContent(_ctx: Context): string {
  ...
  const basics = ui.div("flex flex-col gap-2")(
    ...
    row(
      "Disabled",
      ui.IText("Z").Disabled().Placeholder("Cannot type").Render("Disabled"),
    ),
    row(
      "With preset value",
  ...
  const styling = ui.div("flex flex-col gap-2")(
    row(
      "Wrapper .Class()",
      ui.IText("C1").Class("p-2 rounded bg-yellow-50").Render("Styled wrapper"),
    ),
    row(
      "Label .ClassLabel()",
      ui
    ...
    row(
      "Input .ClassInput()",
      ui.IText("C3").ClassInput("bg-blue-50").Render("Custom input background"),
    ),
