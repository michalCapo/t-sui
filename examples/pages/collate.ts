...
import { TQuery, TField, BOOL, DATES, SELECT, createCollate, CollateGray } from "../../ui.data";
...
const init: TQuery = { Limit: 10, Offset: 0, Order: "createdat desc", PendingOrder: "", Search: "", Filter: [] };
...
export function CollateContent(ctx: Context): string {
    ...
    // Configure collate with chainable API
    collate
        .setColor(CollateGray)
        .setSearch([Name, Email, City])
        .setSort([Name, Email, CreatedAt])
        .setFilter([Active, CreatedAt, Role])
        .Row(function (r: Row, _: number): string {
