...
function render(ctx: Context, f: DemoForm, err?: Error): string {
	function Submit(inner: Context): string {
		inner.Body(f);
		inner.Success("Form submitted successfully");
		return render(inner, f, undefined);
	}
	...
	return ui.div(
		...
		ui.form(
			"flex flex-col gap-4 bg-white p-6 rounded-lg shadow w-full",
			demoTarget,
			ctx.Submit(Submit).Replace(demoTarget),
		)(
