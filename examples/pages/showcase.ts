...
function render(ctx: Context, f: DemoForm, err?: Error): string {
    function Submit(inner: Context): string { inner.Body(f); inner.Success('Form submitted successfully'); return render(inner, f, undefined); }
    ...
    return ui.div('grid gap-4 sm:gap-6 items-start w-full', demoTarget)(
        ui.form('flex flex-col gap-4 bg-white p-6 rounded-lg shadow w-full', demoTarget, ctx.Submit(Submit).Replace(demoTarget))(
            ui.div('text-xl font-bold')(
                'Component Showcase Form'
            ),
            (function () { let v = ''; if (err) { v = ui.div('text-red-600 p-4 rounded text-center border-4 border-red-600 bg-white')(err.message); } return v; })(),
            ...
            ui.div('hidden sm:block overflow-x-auto')(
                ui.IRadioButtons('Gender', f)
                    .Options(genders)
                    .Render('Gender'),
            ),
            ui.ISelect('Country', f)
                .Options(countries)
                .Placeholder('Select...')
                .Render('Country'),
            ui.ICheckbox('Agree', f)
                .Required()
                .Render('I agree to the terms'),

            ui.IDate('BirthDate', f)
                .Render('Birth Date'),
            ui.ITime('AlarmTime', f)
                .Render('Alarm Time'),
            ui.IDateTime('Meeting', f)
                .Render('Meeting (Local)'),
