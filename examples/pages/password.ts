...
export function PasswordContent(_ctx: Context): string {
  ...
  const basics = ui.div("flex flex-col gap-2")(
    ...
    row(
      "Readonly",
      ui.IPassword("P4").Readonly().Value("secret").Render("Readonly password"),
    ),
    row(
      "Disabled",
    ...
    row(
      ...
      ui.IPassword("P7").Type("text").Value("visible value").Render("As text"),
    ),
  );
  ...
  const behavior = ui.div("flex flex-col gap-2")(
    row(
      ...
      ui.IPassword("A1").Autocomplete("new-password").Render("New password"),
    ),
    row(
      "Pattern (min 8 chars)",
      ui
