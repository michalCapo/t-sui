...
export function DateContent(_ctx: Context): string {
  ...
  const basics = ui.div("flex flex-col gap-2")(
    ...
    row(
      "Readonly time",
      ui.ITime("RO").Readonly().Value("10:00").Render("Readonly time"),
    ),
    row(
      "Disabled datetime",
      ui.IDateTime("D").Disabled().Render("Disabled datetime"),
    ),
  );
  ...
  const styling = ui.div("flex flex-col gap-2")(
    row(
      "Wrapper .Class()",
      ui.IDate("C").Class("p-2 rounded bg-yellow-50").Render("Styled wrapper"),
    ),
    row(
      "Label .ClassLabel()",
      ui
  ...
  const behavior = ui.div("flex flex-col gap-2")(
    ...
    row(
      ...
      ui
        .IDateTime("Click")
        .Click("console.log('clicked datetime')")
        .Render("On click, log"),
    ),
  );
