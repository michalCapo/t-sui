...
function open(tag: string) {
  return function (css: string, ...attr: Attr[]) {
    return function (...elements: string[]) {
      ...
      return (
        "<" + tag + " " + attrsStr + ">" + elements.join(" ") + "</" + tag + ">"
      );
    };
  };
}
  ...
  return Object.assign(base, {
    Render: function (label: string): string {
      ...
      return div(base._state.css)(
        Label(base._state.cssLabel, {
          for: base._state.target.id,
          required: base._state.required,
        })(label),
        input(
          Classes(
            INPUT,
            base._state.size,
            base._state.cssInput,
            base._state.disabled && DISABLED,
          ),
          {
            id: base._state.target.id,
            name: base._state.name,
            type: base._state.as,
            onchange: base._state.onchange,
            onclick: base._state.onclick,
            required: base._state.required,
            disabled: base._state.disabled,
            value: value,
            pattern: base._state.pattern,
            placeholder: base._state.placeholder,
            autocomplete: base._state.autocomplete,
          },
        ),
      );
    },
  ...
  const api = {
    ...
    Render: function (text: string): string {
      const selected = state.data
        ? String((state.data as Record<string, unknown>)[state.name] || "")
        : "";
...
function IRadioButtons(name: string, data?: any) {
  ...
  const api = {
    ...
    Render: function (text: string): string {
      ...
      )(Label("font-bold")("" + text), div("flex gap-2 flex-wrap")(items));
    },
  };
  return api;
}
        ...
        for (let m = 0; m < state.cellAttrs[lastRowIndex].length; m++) {
          ...
          if (a) {
            ...
            if (idx >= 0) {
              ...
              if (k2 < a.length && (a[k2] === '"' || a[k2] === "'")) {
                k2++;
              }
              let num = "";
              while (k2 < a.length && a[k2] >= "0" && a[k2] <= "9") {
                num += a[k2];
                k2++;
              }
              const n = Number(num);
    ...
    Render: function (): string {
      ...
      for (let rowIndex = 0; rowIndex < state.rows.length; rowIndex++) {
        ...
        for (let j = 0; j < row.length; j++) {
          const cell = row[j];
          let cls = "";
          if (j < state.colClasses.length && state.colClasses[j] !== "") {
        ...
        for (let k = usedCols; k < state.cols; k++) {
          let cls = "";
          if (k < state.colClasses.length && state.colClasses[k] !== "") {
